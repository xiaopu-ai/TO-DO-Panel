...
contextBridge.exposeInMainWorld('notchAPI', {
  ...
  getAppIcons: (paths) => ipcRenderer.invoke('apps:icons', paths),
  pickApp: () => ipcRenderer.invoke('apps:pick'),
  launchApp: (appPath) => ipcRenderer.invoke('apps:launch', appPath),
  ...
  onChatChunk: (cb) => subscribe('chat:chunk', (event, payload) => cb(payload)),
  ...
  setChatConfig: (config) => ipcRenderer.invoke('chat:set-config', config),
